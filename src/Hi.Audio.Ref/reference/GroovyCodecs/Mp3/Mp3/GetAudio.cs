...
namespace Hi.Audio.Ref.GroovyCodecs.Mp3.Mp3
{
    ...
    internal class GetAudio
    {
        ...
        internal void init_inStream(LameGlobalFlags gfp, Stream inStream, Enc enc)
        {
            count_samples_carefully = false;
            num_samples_read = 0;
            pcmbitwidth = parse.in_bitwidth;
            pcmswapbytes = parse.swapbytes;
            pcm_is_unsigned_8bit = !parse.in_signed;
            musicin = OpenSndStream(gfp, inStream, enc);
        }
        ...
        private Stream OpenSndStream(LameGlobalFlags gfp, Stream inStream, Enc enc)
        {
            gfp.num_samples = -1;
            musicin = inStream;
            ...
            if (is_mpeg_file_format(parse.input_format))
            {
                ...
                {
                    //throw new Exception(string.Format("Error reading headers in mp3 input stream."));
                }
        ...
        private Stream OpenSndFile(LameGlobalFlags gfp, string inPath, Enc enc)
        {
            gfp.num_samples = -1;
            try
            {
