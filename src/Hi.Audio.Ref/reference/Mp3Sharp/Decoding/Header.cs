...
namespace MP3Sharp.Decoding {
    ...
    public class Header {
        ...
        internal Header(AudioFormat audioFormat)
        {
            for(var i=0; i< Frequencies.Length; i++)
            {
                var ix = Frequencies[i].ToList().FindIndex(f => f == audioFormat.SampleRate);
                if (ix != -1)
                {
                    _Version = i;
                    _SampleFrequency = ix;
            ...
            _NumberOfSubbands = 1;
            _Copyright = false;
            Framesize = audioFormat.ChunkLength;
            _BitrateIndex = 1;
            _Headerstring = -281536;
            _Original = false;
            _Syncmode = 1;
            _ProtectionBit = 1;
            _PaddingBit = 0;
